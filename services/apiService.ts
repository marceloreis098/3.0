...
const handleResponse = async (response: Response) => {
    ...
    try {
        ...
        if (!response.ok) {
            // If it's an error response and not JSON, throw the text as the error message
            throw new Error(text || response.statusText);
        }
        // If it's a success response but not JSON, this is unexpected
        console.error("API returned a successful but non-JSON response:", text);
...
export const addUser = async (user: Omit<User, 'id' | 'lastLogin' | 'avatarUrl' | 'ssoProvider'>, username: string): Promise<User> => {
    const response = await fetch(`${API_BASE_URL}/users`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
...
export const deleteUser = async (userId: number, username: string): Promise<void> => {
...
export const getEquipmentHistory = async (id: number): Promise<EquipmentHistory[]> => {
...
export const checkApiStatus = async (): Promise<{ ok: boolean, message?: string }> => {
    try {
        ...
        return { ok: false, message: `O servidor respondeu com o status ${response.status}.` };
    } catch (error) {
        return { ok: false, message: 'Não foi possível conectar à API. Verifique se o servidor backend está em execução e acessível.' };
    }
};

export const getTermoTemplates = async (): Promise<{ entregaTemplate: string | null; devolucaoTemplate: string | null; }> => {
...
// PRODUCT MANAGEMENT & LICENSE TOTALS
...
export const importLicenses = async (importData: LicenseImportData, username: string): Promise<{ success: boolean, message: string }> => {
